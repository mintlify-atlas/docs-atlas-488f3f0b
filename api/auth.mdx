---
title: "Auth — WABotJS Credential Management Reference"
sidebarTitle: "Auth"
description: "Auth manages SQLite-backed WhatsApp session credentials and Baileys signal keys, with get/set/del helpers for the auth store."
---
...
Deletes **only** the `'creds'` row from the SQLite store without dropping the database file or the Signal key material. Resets the in-memory `creds` reference and the loaded flag.

Use `dropCreds()` when you want to force a re-authentication (new QR or OTP) while preserving other stored keys.

<Note>
  After calling `dropCreds()`, calling `state` throws until `load()` is called
  again, which will generate fresh credentials via `baileys.initAuthCreds()`.
</Note>
